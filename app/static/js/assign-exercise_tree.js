        ...
        .then((data) => {
          ...
          if (data.Success) {
            ...
            if (teamId && teamId !== "null") {
              // If a team is assigned, fetch athletes in the team
            ...
            } else if (athleteId && athleteId !== "null") {
              // If an individual athlete is assigned, send a single notification
              sendNotificationToAthlete(
                athleteId,
                coachId,
                workOutName,
                datePicker.value
...
function sendNotificationsToTeam(teamId, coachId, workoutName, date) {
  fetch(`/getAthletesForTeam?teamId=${teamId}`)
    ...
    .catch((error) => {
      console.error("Error fetching athletes for team:", error);
    });
}

function sendNotificationToAthlete(athleteId, coachId, workoutName, date) {
