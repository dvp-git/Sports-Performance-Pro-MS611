...
assignExerciseBtn.addEventListener("click", function (e) {
  e.preventDefault();
  if (teamId)
    window.location.href = `/coachAthleteTest?teamId=${teamId}&coachId=${coachId}&date=${date}#return-button`;
  else if (athleteId)
    window.location.href = `/coachAthleteTest?athleteId=${athleteId}&coachId=${coachId}&date=${date}#return-button`;
});
...
async function fetcWorkoutforAthletebyTeamsByCoach(
  ...
  const response = await fetch(
    `/getTestWorkoutforAthletebyTeamsByCoach?athleteId=${athleteId}&coachId=${coachId}&date=${selectedDate}`
  );
  const data = await response.json();
  //console.log("These are the workouts : ", data);
  return data;
...
function displayWorkouts(workouts, container, athleteId) {
  ...
  const workoutCompletionPromises = workouts.map((workout) =>
    fetch(`/getTestBlocksByTestWorkout?workoutId=${workout.test_workout_id}`)
      .then((response) => response.json())
      .then((data) => {
        if (data && data.blocks) {
          const blockCompletionPromises = data.blocks.map((block) =>
            fetchBlockCompletion(athleteId, block.test_block_id)
          );
  ...
  Promise.all(workoutCompletionPromises).then((averageCompletions) => {
    workouts.forEach((workout, index) => {
      ...
      const combinedWorkoutId = `${athleteId}-${workout.test_workout_id}`;
      ...
      const completionPercentages = await Promise.all(
        data.exercises.map(async (exercise) => {
          const loadResponse = await fetch(
            `/showTestAthleteExerciseInputLoads?exerciseId=${exercise.test_exercise_id}&athleteId=${athleteId}`
          );
  ...
  fetch(`/getTestBlocksByTestWorkout?workoutId=${workoutId}`)
    ...
    .then((data) => {
      ...
      blocks.forEach((block) => {
        ...
        const combinedBlockId = `${athleteId}-${block.test_block_id}`;
        ...
        fetch(`/getTestExercisesByTestBlock?blockId=${block.test_block_id}`)
          .then((response) => response.json())
          .then((data) => {
  ...
  fetch(`getTestExercisesByTestBlock?blockId=${blockId}`)
    ...
    .then((data) => {
      ...
      exercises.forEach((exercise) => {
        ...
        const combinedExerciseId = `${athleteId}-${exercise.test_exercise_id}`;
...
async function viewAssignedExercise(exerciseElement, combinedExerciseId) {
  ...
  try {
    const response = await fetch(
      `getTestExerciseDetails?exerciseId=${exerciseId}`
    );
    const data = await response.json();
    ...
    exercises.forEach(async (exercise, index) => {
      ...
      for (let i = 0; i < loadsReps["coach"].length; i++) {
        ...
        try {
          const loadResponse = await fetch(
            `/showTestAthleteExerciseInputLoads?exerciseId=${exerciseId}&athleteId=${athleteId}`
          );
          const loadData = await loadResponse.json();
          console.log(
...
async function displayTeamsAsTree() {
  ...
  data.teams.forEach((team) => {
    ...
    fetch(
      `/getTestWorkoutsByTeam?teamId=${team.team_id}&date=${currentDate}&coachId=${coachId}`
    )
      .then((response) => response.json())
  ...
  data.athletes.forEach((athlete) => {
    ...
    athleteCaret.addEventListener("click", async function () {
      ...
      if (!athleteWorkoutsContainer.hasChildNodes()) {
        try {
          const workoutsResponse = await fetch(
            `/getTestWorkoutsByAthleteDirect?athleteId=${athleteId}&date=${currentDate}&coachId=${coachId}`
          );
...
async function expandTeamDetails(teamId, date) {
  ...
  for (const athlete of athletesData.athletes) {
    ...
    const workoutsResponse = await fetch(
      `/getTestWorkoutsByTeam?teamId=${teamId}&date=${date}&coachId=${coachId}`
    );
    const workoutsData = await workoutsResponse.json();
    if (!workoutsData.error && workoutsData.length > 0) {
      displayWorkouts(
...
async function expandAthleteWorkouts(athleteId, date) {
  ...
  try {
    const workoutsResponse = await fetch(
      `/getTestWorkoutsByAthleteDirect?athleteId=${athleteId}&date=${date}&coachId=${coachId}`
    );
    if (workoutsResponse.ok) {
      const workoutsData = await workoutsResponse.json();
