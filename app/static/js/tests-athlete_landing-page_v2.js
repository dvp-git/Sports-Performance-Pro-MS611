...
async function fetcTesthWorkoutsByAthleteDirect(
  athlete_id,
  coachId,
  selectedDate
) {
  const response = await fetch(
    `getTestWorkoutsByAthleteDirect?athleteId=${athlete_id}}&date=${selectedDate}&coachId=${coachId}`
  );
  const data = await response.json();
...
async function fetchTestWorkoutsForAthleteAndTeams(athleteId, teamIds) {
  try {
    // Once you have athleteIds and teamIds, fetch workouts
    const workoutsForAthlete = await fetchTestWorkouts(athleteId);
    console.log("Tests By Athlete:", workoutsForAthlete);

    const workoutsForTeams = [];
    for (const teamId of teamIds) {
      //console.log("Team ID ", teamId);
      const teamWorkouts = await fetchTestWorkouts(null, teamId);
      workoutsForTeams.push(teamWorkouts);
    }
    console.log("Tests By Teams:", workoutsForTeams);
...
function displayWorkouts(workouts) {
  ...
  exerciseDetails.innerHTML = "Select a Test to view details.";
  ...
  defaultOption.text = "Choose a Test";
  ...
  workouts.forEach((workout) => {
    ...
    option.value = workout.test_workout_id;
  ...
  workoutDropdown.addEventListener("change", function () {
    ...
    console.log(
      `Selected Test: ${selectedOption.text}, ID: ${selectedOption.value}`
    );
    // Handle the workout selection
    // Example: selectedWorkout = selectedOption.value; // or any other logic
    fetchTestBlocksForTestWorkout(selectedWorkout);
  });
}

// Function to fetch and display blocks for a selected workout
function fetchTestBlocksForTestWorkout(workoutId) {
  fetch(`/getTestBlocksByTestWorkout?workoutId=${workoutId}`)
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      if (data.blocks && data.blocks.length > 0) {
        blockIds = data.blocks.map((block) => block.test_block_id);
        displayBlocks(data.blocks);
        // Automatically select the first block to display its exercises
        fetchTestExercisesForTestBlock(data.blocks[0].test_block_id);
      } else {
        exerciseDetails.innerHTML = "No  blocks found for this test.";
      }
    })
...
workoutTabs.addEventListener("click", (e) => {
  if (e.target.tagName == "BUTTON") {
    ...
    console.log(`Clicked  Test  button`, e.target);
...
function displayBlocks(blocks) {
  ...
  blocks.forEach((block) => {
    ...
    option.value = block.test_block_id;
  ...
  blockDropdown.addEventListener("change", function () {
    ...
    const selectedOption = this.options[this.selectedIndex];
    console.log(
      `Selected Block: ${selectedOption.text}, ID: ${selectedOption.value}`
    );

    // Call a function to handle the block selection
    // This function should fetch and display exercises for the selected block
    fetchTestExercisesForTestBlock(selectedOption.value);
  });
}

async function fetchTestExercisesForTestBlock(blockId) {
  try {
    const response = await fetch(
      `getTestExercisesByTestBlock?blockId=${blockId}`
    );
    const data = await response.json();
    if (data.exercises) {
...
function displayExercises2(blockId) {
  ...
  fetch(`getTestExercisesByTestBlock?blockId=${blockId}`)
    .then((response) => response.json())
    .then((data) => {
      ...
      function showExercise(index) {
        ...
        viewAssignedExercise(
          exercises[index].test_exercise_id,
          exerciseContainer
        );

        // Log the exercise ID and name
        console.log(
          `Exercise displayed: ID ${exercises[index].test_exercise_id}, Name: ${exercises[index].name}`
        );
      }
      ...
      prevButton.addEventListener("click", async function () {
        ...
        if (currentExerciseIndex > 0) {
          ...
          currentBlockIndex--;
          await fetchTestExercisesForTestBlock(blockIds[currentBlockIndex]);
          currentExerciseIndex = currentExercises.length - 1;
          showExercise(currentExerciseIndex);
        }
        console.log(
          `Current Block Index: ${currentBlockIndex}, Current Exercise Index: ${currentExerciseIndex}`
        );
      });
      ...
      nextButton.addEventListener("click", function () {
        ...
        console.log(
          `Indexes: Exercise - ${currentExerciseIndex}, Block - ${currentBlockIndex}`
        );
        ...
        if (
          ...
          if (currentExerciseIndex < exercises.length - 1) {
            ...
            fetchTestExercisesForTestBlock(blockIds[currentBlockIndex]);
          }
        } else {
      ...
      function displayCompletionMessage() {
        ...
        exerciseDetails.innerHTML =
          "You have completed all exercises in this Test.";
        ...
        completeButton.onclick = async function () {
          ...
          if (selectedWorkout) {
            ...
            console.error("Test ID could not be determined.");
...
function viewAssignedExercise(exerciseId, containerElement) {
  ...
  fetch(`getTestExerciseDetails?exerciseId=${exerciseId}`)
    .then((response) => response.json())
    .then((data) => {
      ...
      exercises.forEach((exercise, index) => {
        ...
        fetch(
          `/showTestAthleteExerciseInputLoads?exerciseId=${currentExerciseId}&athleteId=${athleteId}`
        )
          .then((response) => {
...
async function initialData() {
  try {
    ...
    myWorkouts = await fetchTestWorkoutsForAthleteAndTeams(athleteId, teamIds);
    console.log("These are my Tests: initial Data ", myWorkouts);
      ...
      personalCoaches.forEach((coach) => {
        trainingTable.row
          .add({
            ...
            name: `My individual Tests-${coach.name}`,
            sport: "sport",
            team_id: null,
          })
          .draw(false);
...
$("#trainingTable tbody").on("click", "tr", function () {
  ...
  currentWorkouts =
    currentTeamId === null
      ? fetch(
          `getTestWorkoutsByAthleteDirect?athleteId=${athleteId}&coachId=${currentCoachId}&date=${currentDate}`
        )
          .then((response) => {
            response = response.json();
            return response;
          ...
          .then((workout_data) => {
            ...
            console.log("Current Tests:", currentWorkouts);
      ...
      : fetch(
          `getTestWorkoutsByTeam?teamId=${currentTeamId}&coachId=${currentCoachId}&date=${currentDate}`
        )
          .then((response) => response.json())
          .then((data) => {
            console.log("This is my data:", data);
